...
function area(data) {
    ...
    var margin2 = {top: 100, right: 40, bottom: 100, left: 40},
    margin = {top: areaDiv.height() - 50, right: 40, bottom: 20, left: 40},
	...
	d3.time.format.utc("data/test.csv");//Complete the code   2016-01-13 T 17:01:29.450 Z
    ...
    var area = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                return x(format.parse(d.date));//Complete the code
            })
            .y0(height)
            .y1(function (d) {
                return y(d.id);//Complete the code
            });
	...
	d3.csv("data/test.csv", function(error, data) {
		...
		y.domain([d3.min(data.map(function(d) { return d.id; })), d3.max(data.map(function(d) { return d.id; }))]);//Complete the code
